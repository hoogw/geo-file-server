...
// **************   geo-file-server   *****************  
      ...
      //const url_template_base_googlemaps_vector = template_protocol +  '//' + window.location.hostname + ':' + _port_http_https  +'/googlemaps2/default?';
      //const url_template_base_googlemaps_vector = 'http://transparentgov.net:3000/googlemaps2/default?';
      const url_template_base_googlemaps_vector = 'http://localhost:3000/googlemaps2/default?';
      ...
      fs.readdir(dir, (readErr, _files) => {
        ...
        function render(dirs, renderFiles, lolwuts) {
          ...
          let html = `${[
            ...
            `    <title>TransparentGOV gis-server ${he.encode(pathname)}</title>`,
            `    <style type="text/css">${css}</style>`,
            '  </head>',
            '  <body>',
            `<h3>${he.encode(pathname)}</h3>`,
          ].join('\n')}\n`;
            ...
            if (!hidePermissions) {
              html += `<td class="perms"><code>(${permsToString(file[1])})</code></td>`;
                  ...
                  // **************   geo-file-server   *****************     
                              ...
                              html +=      `<td class="file-size"><code>${sizeToString(file[1], humanReadable, si)}</code></td>`;
                              html +=      `<td class="display-name"><a href="${href}">${displayName}</a></td>` ;

                                        if (ext === 'geojson') {
                                                        html += ` &nbsp;&nbsp;&nbsp;`
                                                        html +=          `<td class="display-name">` 
                                                        html +=               `<a target="_blank" href="`+ url_template_base_googlemaps_vector  + `layer=${displayName}&url=http://127.0.0.1:4444${href}">GoogleMaps</a></td>` 
                                        }
          ...
          html += '</table>\n';
          html += `<br><address> TransparentGov.net </address>\n` + '</body></html>';
